...
class FormController {
    ...
    public async Post(req: Request, res: Response) {
        try {
            const { name,phone_number,email ,subject,comment} = req.body

            const form = await AppDataSource.getRepository(FormEntity ).createQueryBuilder().insert().into(FormEntity ).values({ name,phone_number,email ,subject,comment }).returning("*").execute()
    ...
    public async Put(req: Request, res: Response) {
        try {
            ...
            const form = await AppDataSource.getRepository(FormEntity ).createQueryBuilder().update(FormEntity )
                .set({ name,phone_number,email ,subject,comment })
                .where({ id })
                .returning("*")
                .execute()
